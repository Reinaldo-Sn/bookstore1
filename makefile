...
# pyenv detection (cross-platform: uses 'where' on Windows, falls back to default path)
PYENV_ROOT := $(shell pyenv root 2>nul || pyenv root 2>/dev/null)
ifeq ($(PYENV_ROOT),)
PYENV_VERSION_DIR ?= $(HOME)/.pyenv/versions/$(PYTHON_VERSION)
else
PYENV_VERSION_DIR ?= $(PYENV_ROOT)/versions/$(PYTHON_VERSION)
endif
PIP ?= pip3
...
help:  ## Display this help
	@powershell -NoProfile -Command " \
		$$lines = Get-Content '$(MAKEFILE_LIST)'; \
		Write-Host ''; \
		Write-Host 'Usage:'; \
		Write-Host '  make <target>'; \
		foreach ($$line in $$lines) { \
			if ($$line -match '^##@\s*(.+)') { Write-Host ''; Write-Host $$matches[1] } \
			elseif ($$line -match '^([a-zA-Z0-9_-]+):.*##\s*(.+)') { Write-Host ('  {0,-15} {1}' -f $$matches[1], $$matches[2]) } \
		}"
